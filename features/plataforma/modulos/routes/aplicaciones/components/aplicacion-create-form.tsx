...
import { aplicacionCreateAction } from "@/features/plataforma/modulos/shared/actions";
import {
...
type AplicacionCreateFormProps = {
  moduloId: string;
};

export function AplicacionCreateForm({ moduloId }: AplicacionCreateFormProps) {
  const router = useRouter();

  const form = useForm<AplicacionCreateSchema>({
    resolver: zodResolver(aplicacionCreateSchema),
    defaultValues: {
      moduloId,
      clave: "",
      slug: "",
  ...
  async function onSubmit(data: AplicacionCreateSchema) {
    try {
      const response = await aplicacionCreateAction(data);
